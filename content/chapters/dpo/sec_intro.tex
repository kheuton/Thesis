...
To overcome this limitation, many methods have been proposed~\citep{mandi_decision-focused_2024}. We consider two families of methods here. In Perturbation methods~\citep{abernethyPerturbationTechniquesOnline2016,berthetLearningDifferentiablePerturbed2020}, informative gradients are obtained by adding noise to model outputs.
...
In recent years, several methods papers and benchmark studies have begun to offer apples-to-apples comparisons of different methods on synthetic and real problems. However, we find that Perturbation methods have rarely been featured in such experiments.
...
\begin{itemize}[leftmargin=*,noitemsep,topsep=0pt]
    \item We evaluate DPO alongside recent end-to-end learning methods across three experimental settings: \citeauthor{gengBenchmarkingPtOPnO2024}'s benchmarks (Fig.~\ref{fig:bench_bump}), a synthetic dataset from~\citeauthor{huang2024decisionfocusedPG} (Fig.~\ref{fig:noisy_synth_triptych}), and applications to top-K site selection (Fig.~\ref{fig:topK_interpolation_main}).    
