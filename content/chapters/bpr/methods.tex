...
This study was reviewed by the Tufts University Health Sciences Institutional Review Board and deemed to be non-human subjects research.
...
\begin{equation}
    \text{RMSE} = \sqrt{\frac{1}{S}\sum_{s = 1}^{S}(y_{s} - \widehat{y_{s}})^{2}},\ \ \ \ \text{MAE} = \frac{1}{S}\sum_{s = 1}^{S}|y_{s} - \widehat{y_{s}}|
\end{equation}
...
. RMSE is more sensitive to outliers because it squares large errors.
...
\begin{equation}
    \text{BPR} = \frac{\sum_{k \in I}^{}y_{k}}{\sum_{k \in \text{TopKInds(y)}}y_{k}} = \frac{\text{$\#$ events in K regions picked by model}}{\text{$\#$ events in actual K highest count regions}}
\end{equation}
...
\begin{equation}
    y|\ x,\bm{\theta}\  \sim \ \text{Poisson}(\lambda\  = e^{\bm{\theta}^{T}x})\
\end{equation}
